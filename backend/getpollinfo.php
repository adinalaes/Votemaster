<?php 
    ...
    $SQL_polls = "SELECT * FROM polls WHERE poll_id = " . $_POST["poll_id"] . " AND active = 1 AND city_id=" . $_SESSION["city"];
    ...
    $response = array(
        "status" => true,
        "code" => 0,
        "data" => array("poll_info" => array('city_id'=>$row['city_id'], 'poll_name'=>$row['poll_name'], 'poll_description'=>$row['poll_description']), "poll_options" => array())
    );
